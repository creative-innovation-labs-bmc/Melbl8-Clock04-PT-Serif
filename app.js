...
function findTimeRange(text) {
  const candidates = [targetTime, currentTime]
    .filter(Boolean)
  ...
  const leads = [targetLead, currentLead]
    .filter(Boolean)
    .sort((a, b) => b.length - a.length);
...
function applyFontSize(size, duration = 0, easing = 'linear', reason = 'instant') {
  ...
  if (scaleEvents.length > 30) scaleEvents = scaleEvents.slice(-30);
}
...
function buildEditPlan(from, to) {
  ...
  return {
    ...
    suffixText: suffix ? from.slice(from.length - suffix) : ''
  };
}
...
async function typeInitial(target, token) {
  ...
  applyFontSize(measureFontSize(target), 0, 'linear', 'initial');
...
async function editSentence(target, token) {
  ...
  if (noAnimation || reducedMotion) {
    applyFontSize(targetSize, 0, 'linear', 'reduced');
    render(target, '');
    renderedText = target;
    setCursorWorking(false);
    return;
...
function updateClock(now = performance.now()) {
  ...
  const progress = clamp((state.second + fractionalSecond) / 60, 0, 1);
